...
let githubOutput = process.env.GITHUB_STATE;
...
async function run() {
    ...
    githubOutput += `name=issues::${issueIDs.map(item => `resolve: #${item}`).join(' ')}\n`;
    process.env.GITHUB_STATE = githubOutput;
    console.log(`name=issues::${issueIDs.map(item => `resolve: #${item}`).join(' ')}`);
}
